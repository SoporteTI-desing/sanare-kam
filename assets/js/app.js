...
function closeModal(){ document.querySelector('.modal').classList.remove('open'); document.removeEventListener('keydown', escClose); }
...
async function init(){
  ...
  if(!document.querySelector('.modal')){{ const m = document.createElement('div'); m.className='modal'; document.body.appendChild(m); }}
  $$('.slide').forEach((el, idx)=>{
    el.style.cursor = 'zoom-in';
    el.addEventListener('click', (e)=>{ if(e.target.closest('.whatsapp')) return; openModal(data[idx]); });
  });
  $$('.card').forEach((el, idx)=>{
    el.style.cursor = 'zoom-in';
    el.addEventListener('click', (e)=>{ if(e.target.closest('.whatsapp')) return; openModal(data[idx]); });
